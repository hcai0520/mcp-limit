...
JOB_LIST=${JOB_LIST:-job_list_production.csv}
JOB_ID=${1:-${SLURM_ARRAY_TASK_ID:-}}
EXEC=${EXEC:-./build-gcc14/pb_fww_production}
...
IFS='|' read -r seed trials mass geometry emitter mode output job_id <<< "$row"
...
"$EXEC" \
    "$seed" 1 "$trials" "$mass" "$geometry" "$emitter" "$mode" "$output" \
    --beam-energy "${BEAM_ENERGY:-120}" \
    --epsilon "${EPSILON:-0.01}" \
    --sigma-inelastic-mb "${SIGMA_INELASTIC_MB:-38.4538}" \
    --interaction-ratio "${INTERACTION_RATIO:-1.0}" \
    --m-pair-max "${M_PAIR_MAX:-12.0}" \
